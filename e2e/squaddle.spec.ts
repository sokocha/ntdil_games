...
test.describe('Squaddle Game', () => {
  ...
  test('should load the game page', async ({ page }) => {
    ...
    const closeButton = page.locator('button:has-text("Got it"), button:has-text("Start"), button:has-text("Close"), button:has-text("×"), button:has-text("Let\'s Play!"), button:has-text("Skip tutorial")')
  ...
  test('should show onboarding modal for first-time users', async ({ page }) => {
    await page.goto('/squaddle')
    // Check if onboarding modal appears (looking for Welcome text or modal container)
    const modal = page.locator('text=Welcome to Squaddle')
  ...
  test('should display game clues', async ({ page }) => {
    ...
    // Check for clue elements (look for "Next Clue" button or revealed clues)
    const clues = page.locator('text=/Next Clue|Revealed/')
  ...
  test('should have a submit button', async ({ page }) => {
    ...
    await page.waitForTimeout(1000)
    // Look for submit button (checkmark button next to input)
  ...
  test('should allow submitting a guess', async ({ page }) => {
    ...
    // Submit the guess (checkmark button)
    const submitButton = page.locator('button:has-text("✓")')
  ...
  test('should show round indicator', async ({ page }) => {
    ...
    // Check for round indicator (Easy/Medium/Hard difficulty badges)
    const roundText = page.locator('text=/Easy|Medium|Hard/i')
